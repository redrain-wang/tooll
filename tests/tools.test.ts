...
describe('Core Utility Functions', () => {
  ...
  describe('Base64', () => {
    it('should encode and decode', () => {
      ...
      expect(decode(encode(testStr))).toBe(testStr)
    })
  ...
  describe('URL Encoding', () => {
    it('should encode and decode URLs', () => {
      const testUrl = 'https://example.com?q=hello world'
      expect(decodeURIComponent(encodeURIComponent(testUrl))).toBe(testUrl)
    })
...
describe('API Integration', () => {
  ...
  it('should mock translation API', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ translatedText: 'The weather is nice today' })
    })

    const res = await fetch('/api/translate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ q: '今天天气不错', source: 'zh-cn', target: 'en' })
    })
    const data = await res.json()
    expect(data.translatedText).toBe('The weather is nice today')
  })

  it('should mock RSA generation API', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
    ...
    const data = await res.json()
    expect(data.public).toContain('BEGIN PUBLIC KEY')
    expect(data.private).toContain('BEGIN PRIVATE KEY')
  })
})
